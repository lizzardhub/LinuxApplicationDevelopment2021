...
{
    int test_case = TEST_ALL;
    if (argc > 1) {
        // select which test to run
        test_case = strtol(argv[1], NULL, DECIMAL);
    }
    if (test_case == TEST_INIT || test_case == TEST_ALL)
        test_initialization();
    if (test_case == TEST_PUSH || test_case == TEST_ALL)
        test_push();
    if (test_case == TEST_GROW || test_case == TEST_ALL)
        test_grow();
    if (test_case == TEST_POP || test_case == TEST_ALL)
        test_pop();
    if (test_case == TEST_MEM_FAIL || test_case == TEST_ALL)
        test_mem_fail();
